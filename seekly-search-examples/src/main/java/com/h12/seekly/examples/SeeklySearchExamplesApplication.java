...
/**
 * Main Spring Boot application for the Seekly search examples.
 * Demonstrates how to use the search framework with example entities.
 */
@Slf4j
...
public class SeeklySearchExamplesApplication {
    ...
    private void runPostgresDemo() {
        log.info("Running PostgreSQL Search Engine Demo...");

        try {
            // Create PostgreSQL search engine for products
            SearchEngine<Product> productEngine = SearchEngineFactory.createPostgresSearchEngine(
                    com.h12.seekly.config.PostgresSearchConfig.builder()
                            ...
                            .dbUsername(System.getenv("POSTGRES_USERNAME"))
                            .dbPassword(System.getenv("POSTGRES_PASSWORD"))
                            .entityType("product")
            ...
            // Create PostgreSQL search engine for sellers
            ...
            log.info("PostgreSQL demo completed successfully!");

        } catch (Exception e) {
            log.error("PostgreSQL demo failed", e);
        }
    }

    private void runLuceneDemo() {
    ...
    private void performProductSearches(SearchEngine<Product> engine) {
        ...
        // Basic search
        SearchResponse<Product> response1 = engine.search("iPhone",
        ...
        // Search by brand
        SearchResponse<Product> response3 = engine.search("Apple",
                SearchOptions.builder().maxResults(10).build());
        log.info("Search for 'Apple': Found {} results", response3.getTotalHits());

        // Search by category
        SearchResponse<Product> response4 = engine.search("laptop",
                SearchOptions.builder().maxResults(5).build());
    ...
    private void performSellerSearches(SearchEngine<Seller> engine) {
        ...
        // Basic search
        SearchResponse<Seller> response1 = engine.search("TechStore",
                SearchOptions.builder().maxResults(10).build());
        ...
        // Search by location
        SearchResponse<Seller> response3 = engine.search("California",
                SearchOptions.builder().maxResults(10).build());
        log.info("Search for 'California': Found {} results", response3.getTotalHits());

        // Search by specialty
        SearchResponse<Seller> response4 = engine.search("sports",
                SearchOptions.builder().maxResults(5).build());
